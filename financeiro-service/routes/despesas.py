...
def despesas(
    ...
    hit = cache_get("despesas", cache_key)
    if hit is not None:
        return hit
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        f_resumo  = pool.submit(_resumo_cc)
        f_detalhe = pool.submit(_detalhe)
        result = {"resumoPorCC": f_resumo.result(), "detalhe": f_detalhe.result()}

    cache_set("despesas", cache_key, result)
    return result
