...
from services.microsoft_graph import GraphClient, build_auth_url, exchange_code_for_tokens, get_access_token
...
def _cleanup_states() -> None:
    ...
    expired = [k for k, (_, ts) in _pending_states.items() if ts < cutoff]
    for k in expired:
...
def get_microsoft_auth_url(current_user: dict = Depends(get_current_user)):
    ...
    _pending_states[state] = (_resolve_user_id(current_user), time.time())
