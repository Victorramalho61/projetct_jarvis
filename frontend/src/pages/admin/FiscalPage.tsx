...
interface BennerReconciliationItem {
  chave_acesso: string;
  tipo: string;
  numero: string;
...
interface BennerReconciliation {
  total_nfe_cte: number;
...
type Tab = "dashboard" | "nfse" | "emitidas" | "nfe" | "sync" | "certificados";
...
export default function FiscalPage() {
  ...
  // dashboard — indicador "não encontrados no Benner" (NFe/CTe, agregado, todas empresas)
  const [bennerRecon, setBennerRecon] = useState<BennerReconciliation | null>(null);
  ...
  // emitidas tab (NFSe emitidas pela empresa — direcao=emitida, fonte Portal Nacional)
  const [docsEmitidas, setDocsEmitidas]           = useState<NfseDoc[]>([]);
  const [emitidasLoading, setEmitidasLoading]     = useState(false);
  const [emitidasOffset, setEmitidasOffset]       = useState(0);
  const [qEmitidas, setQEmitidas]                 = useState("");
  const [filterEmitidasStatus, setFilterEmitidasStatus]   = useState("");
  const [filterEmitidasCliente, setFilterEmitidasCliente] = useState("");
  const [emitidasDateInicio, setEmitidasDateInicio] = useState(() => {
    const d = new Date(); return new Date(d.getFullYear(), d.getMonth(), 1).toISOString().slice(0, 10);
  });
  const [emitidasDateFim, setEmitidasDateFim]     = useState(() => new Date().toISOString().slice(0, 10));
  const [statsEmitidas, setStatsEmitidas]         = useState<NfseStats | null>(null);
  const [statsEmitidasLoading, setStatsEmitidasLoading] = useState(false);
  const [exportingEmitidasCsv, setExportingEmitidasCsv] = useState(false);
  const [exportingEmitidasXml, setExportingEmitidasXml] = useState(false);
  const [exportEmitidasError, setExportEmitidasError]   = useState("");
  ...
  const exportBennerReconCsv = async () => {
    if (!token) return;
    ...
    try {
      ...
      if (!resp.ok) return;
      downloadBlob(await resp.blob(), "nao_encontrados_benner_nfe_cte.csv");
    } finally {
  ...
  const loadDocs = useCallback(() => {
    if (!token) return;
    const p = new URLSearchParams({ limit: String(DOCS_LIMIT), offset: String(docsOffset), tipo: "NFSe", direcao: "recebida" });
    if (selectedId)        p.set("company_id", selectedId);
    if (q)                 p.set("q", q);
  ...
  // ── Emitidas tab (NFSe emitidas pela empresa) ───────────────────────────────
  const loadDocsEmitidas = useCallback(() => {
    if (!token) return;
    const p = new URLSearchParams({
      limit: String(DOCS_LIMIT), offset: String(emitidasOffset), tipo: "NFSe", direcao: "emitida",
    });
    if (selectedId)             p.set("company_id", selectedId);
    if (qEmitidas)              p.set("q", qEmitidas);
    if (filterEmitidasStatus)   p.set("status", filterEmitidasStatus);
    if (filterEmitidasCliente)  p.set("destinatario_cnpj", filterEmitidasCliente);
    if (emitidasDateInicio)     p.set("data_inicio", emitidasDateInicio);
    if (emitidasDateFim)        p.set("data_fim", emitidasDateFim);
    setEmitidasLoading(true);
    apiFetch<{ data: NfseDoc[] }>(`/api/fiscal/nfse?${p}`, { token })
      .then((r) => setDocsEmitidas(r.data ?? []))
      .catch(() => setDocsEmitidas([]))
      .finally(() => setEmitidasLoading(false));
  }, [token, selectedId, qEmitidas, filterEmitidasStatus, filterEmitidasCliente, emitidasDateInicio, emitidasDateFim, emitidasOffset]);

  useEffect(() => { if (tab === "emitidas") loadDocsEmitidas(); }, [tab, loadDocsEmitidas]);

  const loadStatsEmitidas = useCallback(() => {
    if (!token || !selectedId) return;
    const p = new URLSearchParams({ company_id: selectedId, direcao: "emitida" });
    if (emitidasDateInicio) p.set("data_inicio", emitidasDateInicio);
    if (emitidasDateFim)    p.set("data_fim", emitidasDateFim);
    setStatsEmitidasLoading(true);
    apiFetch<NfseStats>(`/api/fiscal/nfse/stats?${p}`, { token })
      .then(setStatsEmitidas)
      .catch(() => setStatsEmitidas(null))
      .finally(() => setStatsEmitidasLoading(false));
  }, [token, selectedId, emitidasDateInicio, emitidasDateFim]);

  useEffect(() => { if (tab === "emitidas") loadStatsEmitidas(); }, [tab, loadStatsEmitidas]);
  ...
  const exportCsv = async (tipo: string) => {
    ...
    try {
      ...
      if (tipo === "NFSe")   p.set("direcao", "recebida");
      if (exportDateInicio) p.set("data_inicio", exportDateInicio);
      if (exportDateFim)    p.set("data_fim",    exportDateFim);
      if (filterFonte)      p.set("fonte",       filterFonte);
  ...
  const exportEmitidasCsv = async () => {
    if (!token || !selectedId) return;
    setExportEmitidasError("");
    setExportingEmitidasCsv(true);
    try {
      const p = new URLSearchParams({ company_id: selectedId, tipo: "NFSe", direcao: "emitida" });
      if (emitidasDateInicio) p.set("data_inicio", emitidasDateInicio);
      if (emitidasDateFim)    p.set("data_fim",    emitidasDateFim);
      const resp = await fetch(`/api/fiscal/nfse/export/csv?${p}`, {
        headers: { Authorization: `Bearer ${token}` },
      ...
      if (!resp.ok) {
        const err = await resp.json().catch(() => ({}));
        setExportEmitidasError(`Erro ao exportar CSV: ${(err as {detail?: string}).detail ?? `HTTP ${resp.status}`}`);
        return;
      }
      downloadBlob(await resp.blob(), `fiscal_nfse_emitidas_${emitidasDateInicio || "todos"}.csv`);
    } catch (e) {
      setExportEmitidasError(`Erro ao exportar CSV: ${e instanceof ApiError ? e.message : 'Erro inesperado. Tente novamente.'}`);
    } finally { setExportingEmitidasCsv(false); }
  };

  const exportEmitidasXml = async () => {
    if (!token || !selectedId) return;
    setExportEmitidasError("");
    setExportingEmitidasXml(true);
    try {
      const p = new URLSearchParams({ company_id: selectedId, tipo: "NFSe", direcao: "emitida" });
      if (emitidasDateInicio) p.set("data_inicio", emitidasDateInicio);
      if (emitidasDateFim)    p.set("data_fim",    emitidasDateFim);
      ...
      if (!resp.ok) {
        const err = await resp.json().catch(() => ({}));
        setExportEmitidasError(`Erro ao exportar XMLs: ${(err as {detail?: string}).detail ?? `HTTP ${resp.status}`}`);
        return;
      }
      downloadBlob(await resp.blob(), `xmls_nfse_emitidas_${emitidasDateInicio || "todos"}.zip`);
    } catch (e) {
      setExportEmitidasError(`Erro ao exportar XMLs: ${e instanceof ApiError ? e.message : 'Erro inesperado. Tente novamente.'}`);
    } finally { setExportingEmitidasXml(false); }
  };
  ...
  return (
    <div className="space-y-5 p-6 max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                onChange={(e) => { setSelectedId(e.target.value); setDocsOffset(0); setNfeOffset(0); setEmitidasOffset(0); }}
      ...
      <div className={`flex gap-1 p-1 rounded-xl w-fit flex-wrap ${isDark ? "bg-gray-800" : "bg-gray-100"}`}>
        ...
        <button className={tabCls("nfse")}         onClick={() => setTab("nfse")}>NFSe Recebidas</button>
        <button className={tabCls("emitidas")}     onClick={() => setTab("emitidas")}>NFSe Emitidas</button>
      ...
      {tab === "dashboard" && (
        <div className="space-y-5">
          ...
          {/* ── Indicador: documentos não encontrados no Benner (NFe/CTe, agregado, todas empresas) ── */}
          <div className={`rounded-xl border p-5 space-y-4 ${card}`}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => loadBennerRecon(true)}
                  disabled={bennerReconLoading}
                ...
                >
                  {bennerReconLoading ? "Atualizando…" : "Atualizar"}
                </button>
                <button
                  onClick={exportBennerReconCsv}
                  disabled={bennerReconExporting || !bennerRecon?.not_in_benner_count}
            ...
            <p className={`text-xs ${isDark ? "text-gray-400" : "text-gray-500"}`}>
              Compara os NFe/CTe já sincronizados pelo Jarvis (por chave de acesso) contra as chaves cadastradas no Benner.
              NFSe não está incluída — o Benner corporativo não guarda chave de acesso de NFSe pra comparação confiável.
            </p>

            {bennerReconError ? (
              <p className="text-sm text-red-500">{bennerReconError}</p>
            ) : bennerReconLoading && !bennerRecon ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <KPICard
                    ...
                    value={bennerRecon.total_nfe_cte > 0
                      ? `${((bennerRecon.not_in_benner_count / bennerRecon.total_nfe_cte) * 100).toFixed(1)}%`
                      : "—"}
                    isDark={isDark}
                  />
                </div>
      ...
      {/* ════ TAB: NFSe RECEBIDAS (emitidas contra a empresa) ════ */}
      {tab === "nfse" && (
        <div className="space-y-4">
          {!selectedId ? <NoCompanyPrompt isDark={isDark} card={card} /> : (<>
          <p className={`text-xs -mt-1 ${isDark ? "text-gray-400" : "text-gray-500"}`}>
            NFSe emitidas contra {currentCompany?.nome ?? "a empresa selecionada"} (a empresa é a tomadora do serviço).
          </p>
          {/* Filtros */}
      ...
      {/* ════ TAB: NFSe EMITIDAS (pela empresa) ════ */}
      {tab === "emitidas" && (
        <div className="space-y-4">
          {!selectedId ? <NoCompanyPrompt isDark={isDark} card={card} /> : (<>
          <p className={`text-xs -mt-1 ${isDark ? "text-gray-400" : "text-gray-500"}`}>
            NFSe em que {currentCompany?.nome ?? "a empresa selecionada"} é a prestadora do serviço (emitente),
            a partir da sincronização com o Portal Nacional NFS-e. Documentos vindos de sync municipal direto
            aparecem sem XML/cliente disponíveis.
          </p>

          {/* KPIs */}
          {statsEmitidasLoading ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[...Array(4)].map((_, i) => (
                <div key={i} className={`rounded-xl border p-5 h-24 animate-pulse ${card}`} />
              ))}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <KPICard label="Total Emitidas" value={FMT_NUM.format(statsEmitidas.total_notas)} isDark={isDark} />
              <KPICard label="Valor Total"    value={FMT_BRL.format(statsEmitidas.valor_total)} isDark={isDark} />
              <KPICard label="ISS"            value={FMT_BRL.format(statsEmitidas.valor_iss)}   isDark={isDark} />
              <KPICard
                label="Pendentes"
                value={FMT_NUM.format(statsEmitidas.por_status?.pendente ?? 0)}
                sub={`Conferidos: ${FMT_NUM.format(statsEmitidas.por_status?.conferido ?? 0)}`}
                isDark={isDark}
              />
            </div>
          ) : (
            <p className={`text-sm ${isDark ? "text-gray-300" : "text-gray-500"}`}>Nenhum dado para o período selecionado.</p>
          )}
          ...
          <div className={`rounded-xl border p-4 ${card}`}>
            <div className="flex flex-wrap gap-3">
              <input
                type="text"
                placeholder="Busca full-text (cliente, município...)"
                value={qEmitidas}
                onChange={(e) => { setQEmitidas(e.target.value); setEmitidasOffset(0); }}
                className={`${inp} flex-1 min-w-[200px]`}
              />
              <input
                type="text"
                placeholder="CNPJ do cliente/tomador"
                value={filterEmitidasCliente}
                onChange={(e) => { setFilterEmitidasCliente(e.target.value); setEmitidasOffset(0); }}
                className={`${inp} w-44 font-mono`}
              />
              <select
                value={filterEmitidasStatus}
                onChange={(e) => { setFilterEmitidasStatus(e.target.value); setEmitidasOffset(0); }}
                className={`${inp} w-36`}
              >
                <option value="">Todos status</option>
                <option value="pendente">Pendente</option>
              ...
              <div className="flex flex-col gap-0.5">
                <label className={`text-xs ${isDark ? "text-gray-300" : "text-gray-500"}`}>De</label>
                <input type="date" value={emitidasDateInicio} onChange={(e) => { setEmitidasDateInicio(e.target.value); setEmitidasOffset(0); }}
                  className={`${inp} w-32 text-xs py-1.5`} />
              </div>
              <div className="flex flex-col gap-0.5">
                <label className={`text-xs ${isDark ? "text-gray-300" : "text-gray-500"}`}>Até</label>
                <input type="date" value={emitidasDateFim} onChange={(e) => { setEmitidasDateFim(e.target.value); setEmitidasOffset(0); }}
                  className={`${inp} w-32 text-xs py-1.5`} />
              </div>
              <button
                onClick={() => { setEmitidasOffset(0); loadDocsEmitidas(); loadStatsEmitidas(); }}
                className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors"
              >
                Buscar
              </button>
              <div className="flex flex-wrap items-end gap-2 ml-auto">
                <button
                  onClick={exportEmitidasCsv}
                  disabled={exportingEmitidasCsv || !selectedId}
                  title={!selectedId ? "Selecione uma empresa" : "Exportar CSV"}
                  className={`px-3 py-1.5 text-xs rounded-lg flex items-center gap-1.5 font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${isDark ? "bg-emerald-900/30 border border-emerald-700/60 text-emerald-400 hover:bg-emerald-900/50" : "bg-emerald-50 border border-emerald-200 text-emerald-700 hover:bg-emerald-100"}`}
                >
                  {exportingEmitidasCsv
                    ? <span className="w-3 h-3 border-2 border-current border-t-transparent rounded-full animate-spin" />
                    : (<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>)}
                  CSV
                </button>
                <button
                  onClick={exportEmitidasXml}
                  disabled={exportingEmitidasXml || !selectedId}
                  title={!selectedId ? "Selecione uma empresa" : "Baixar XMLs"}
                  className={`px-3 py-1.5 text-xs rounded-lg flex items-center gap-1.5 font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${isDark ? "bg-blue-900/30 border border-blue-700/60 text-blue-400 hover:bg-blue-900/50" : "bg-blue-50 border border-blue-200 text-blue-700 hover:bg-blue-100"}`}
                >
                  {exportingEmitidasXml
                    ? <span className="w-3 h-3 border-2 border-current border-t-transparent rounded-full animate-spin" />
                    : (<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>)}
                  XML
                </button>
              </div>
              {exportEmitidasError && <p className="text-xs text-red-500 mt-1 col-span-full">{exportEmitidasError}</p>}
            </div>
          </div>
          ...
          <div className={`rounded-xl border overflow-hidden ${card}`}>
            ...
            ) : docsEmitidas.length === 0 ? (
              <p className={`px-4 py-12 text-center text-sm ${isDark ? "text-gray-300" : "text-gray-500"}`}>
                Nenhuma NFSe emitida encontrada para os filtros selecionados.
              </p>
            ) : (
              <div className={`divide-y ${isDark ? "divide-gray-700" : "divide-gray-100"}`}>
                {docsEmitidas.map((doc) => (
                  <div
                    key={doc.id}
                    onClick={() => openDetail(doc)}
                    className={`p-4 cursor-pointer transition-colors ${
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`font-semibold text-base leading-tight truncate ${isDark ? "text-white" : "text-gray-900"}`}
                          title={doc.destinatario_nome || "Cliente não disponível"}>
                          {doc.destinatario_nome || (doc.destinatario_cnpj ? fmtCnpj(doc.destinatario_cnpj) : "Cliente não disponível")}
                        </p>
                        {doc.destinatario_cnpj && (
                          <p className={`text-xs font-mono mt-0.5 ${isDark ? "text-gray-300" : "text-gray-500"}`}>
                            {fmtCnpj(doc.destinatario_cnpj)}
                          </p>
                        )}
                        {!doc.xml_content && doc.fonte === "municipal_direto" && (
                          <p className={`text-xs mt-0.5 italic ${isDark ? "text-gray-500" : "text-gray-400"}`}>
                            XML não disponível para esta fonte
                          </p>
                        )}
                      </div>
                      <div className="text-right shrink-0">
            ...
            <div className={`flex items-center justify-between px-4 py-3 border-t text-sm ${isDark ? "border-gray-700" : "border-gray-200"}`}>
              <span className={isDark ? "text-gray-300" : "text-gray-500"}>{docsEmitidas.length} registros · clique para detalhar</span>
              <div className="flex gap-2">
                <button
                  disabled={emitidasOffset === 0}
                  onClick={() => setEmitidasOffset(Math.max(0, emitidasOffset - DOCS_LIMIT))}
                  className={`px-3 py-1 rounded border text-sm disabled:opacity-40 transition-colors ${
                    isDark ? "border-gray-600 hover:bg-gray-700 text-gray-300" : "border-gray-300 hover:bg-gray-50 text-gray-700"
                  }`}
                >← Anterior</button>
                <button
                  disabled={docsEmitidas.length < DOCS_LIMIT}
                  onClick={() => setEmitidasOffset(emitidasOffset + DOCS_LIMIT)}
                  className={`px-3 py-1 rounded border text-sm disabled:opacity-40 transition-colors ${
                    isDark ? "border-gray-600 hover:bg-gray-700 text-gray-300" : "border-gray-300 hover:bg-gray-50 text-gray-700"
                  }`}
                >Próximo →</button>
