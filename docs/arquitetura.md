...
>  
> **Novas melhorias PayFly**: separação entre contratos e pagamentos eventuais, inclusão de parcelas pendentes, filtro ajustado para considerar apenas pagamentos liquidados (`DATALIQUIDACAO IS NOT NULL`), remoção de campo `total_pago` obsoleto em `PayFlySeries`, remoção de restrição `empresa=3` para fornecedores em ambiente de desenvolvimento.  
>  
> **Ajustes operacionais**: alertas WhatsApp no módulo de monitoramento foram desabilitados temporariamente.