...
**Link pré-preenchido (2026-09-23)**: `ms_forms_url` aceita os marcadores `{email}`, `{empresa}`, `{contato}` (crus ou codificados `%7B...%7D`). `email_service.link_formulario(campanha, cliente)` substitui pelos dados do `sat_clientes` (URL-encoded) em todos os e-mails ao cliente (primeiro envio, cobrança, reforço). Como gerar: no Forms, `... → Obter link pré-preenchido`, digitar `{email}`/`{empresa}` nas perguntas de identificação e colar a URL gerada (formato `ResponsePage.aspx?id=...&r<id>=...`) na campanha — link curto `forms.office.com/r/...` não suporta pré-preenchimento. Objetivo: webhook concilia automaticamente sem depender do cliente digitar e-mail/empresa certo. UI (`SatisfacaoEnvioPage.tsx`) mostra se o link tem pré-preenchimento ativo. Sem migration.
...
## Gestão de Acesso — Role "sgi" faltando no dropdown de perfil (2026-09-22)

Vithoria Andrade foi liberada pro módulo "Pesquisa de Satisfação" (`allowed_modules` continha `satisfacao`), mas continuava sem ver o menu mesmo após logout/login e limpeza de cache. Causa: `AppLayout.tsx` filtra a navegação primeiro por **role** (`NAV_ITEMS.roles.includes(user.role)`) e só depois por `allowed_modules` — o item "Pesquisa de Satisfação" exige `role in ["admin", "sgi"]` (role `sgi` criado em `core-service/migrations/003_add_sgi_role.sql` especificamente pro módulo). O usuário estava com `role="user"`, então nunca passava no primeiro filtro, independente do módulo liberado. Corrigido via `UPDATE profiles SET role='sgi'` direto no banco pra desbloquear a usuária na hora.

Causa raiz de fundo: `AccessManagementPage.tsx` nunca teve a opção `sgi` no `<select>` de troca de perfil (só listava admin/rh/gerente/coordenador_supervisor/administrativo_operacional) — ou seja, nenhum admin conseguiria escolher esse role pela tela, mesmo sabendo que ele existe. Adicionada a opção `<option value="sgi">SGI</option>` (+ tipo `Profile.role` e label do badge em `AppLayout.tsx`).

**Lição:** quando um novo role é criado (migration + backend `_VALID_ROLES`), checar se o `<select>` de atribuição de role no frontend foi atualizado junto — são dois pontos de manutenção desacoplados (constraint do banco / enum do backend / opções do dropdown) e só o dropdown é visível pra quem usa a tela.

## AVD — Diretora ainda "não recebia" e-mail + sent_at nunca gravado no reenvio individual (2026-09-23)

Retomando o caso da Andréia Lima (Diretoria, ver seção de 2026-09-21 acima): mesmo depois do fix do `is_self_evaluation`, RH reportou de novo que o time dela não recebeu o e-mail de avaliação. Investigação (tabelas `performance_evaluation_tokens` + `performance_evaluation_overrides`) reconstruiu a linha do tempo real:

1. Os 13 tokens dela (avalia os 13 liderados diretos) foram criados em 10/08 mas nunca enviados — ela é Diretoria (`hierarchy_level=4`), e o disparo em massa (`/cycle/send-tokens`) **pula esse nível por design** (só cria token, não manda e-mail — existe endpoint dedicado pra isso, `/cycle/tokens/send-for-evaluator`).
2. Em 21/09, alguém usou o endpoint certo duas vezes (10:27 e 14:58) — e-mail consolidado (1 mensagem, tabela com os 13 links) foi enviado corretamente, com `sent_at` gravado.
3. Entre 12:35–12:37 e de novo 15:12–15:15 do mesmo dia, dois atendentes de RH diferentes tentaram resolver o mesmo relato usando a ferramenta errada: **"criar nova avaliação"** (`POST /employees/{id}/new-evaluation`, endpoint pra reabrir avaliação já concluída, com justificativa), rodado manualmente 13x pra cada liderado — **invalidando** os tokens que já tinham sido enviados corretamente e criando 13 novos, cada um mandado por e-mail individual (fragmentando 1 e-mail consolidado em 13 avulsos).
4. Bug real encontrado nesse endpoint (`admin.py::create_new_evaluation`): grava `resend_count`/`last_resent_at` quando o envio dá certo, mas **nunca grava `sent_at`/`sent_to_email`** no token (diferente de todo o resto do fluxo de tokens, que grava). Resultado: mesmo com o SMTP aceitando o envio, o painel do RH mostrava esses 13 como "não enviado" pra sempre — reforçando a falsa impressão de que o e-mail nunca saiu.

**Fix**: `create_new_evaluation` passou a gravar `sent_at`/`sent_to_email` junto com `resend_count`, igual ao resto do fluxo. Disparado manualmente um novo envio consolidado (via `send_evaluation_batch_email`, chamado direto no serviço — não pela rota HTTP, pra evitar mintar um JWT de admin) pros 13 tokens válidos da Andréia; `sent_at` confirmado gravado nos 13.

**Lição:** um endpoint de "correção manual" (override do RH) que não atualiza os mesmos campos de auditoria que o fluxo normal cria um ponto cego — o sistema mostra "não enviado" mesmo quando o e-mail saiu, e isso empurra quem está tentando ajudar a repetir a ação (piorando o problema, como aconteceu aqui: 26 invalidações em vez de 1 reenvio correto) em vez de expor a causa real. Sempre que existir mais de um caminho de código que grava o mesmo tipo de evento (aqui: "token de avaliação enviado"), os campos de auditoria devem ser gravados de forma consistente em todos eles.
...
**Status "ABERTA" + limpeza do histórico (2026-09-24):**
