...
> **Atualizações recentes**: Intervalos de health checks reduzidos para 5 minutos, polling de agentes ajustado para 60 segundos, polling SSE alterado de 2s para 5s, validação de priorização (priority/effort/risk) e limitação a 3 propostas por ciclo no módulo de agentes, correção em filtro Freshservice que gerava erro 400, otimização com merge de queries em agent_runs, implementação do cruzamento Jarvis×Benner, verificação de aderência contratual, cálculo de totais financeiros e dashboard de oportunidades no módulo de governança. Correção crítica na autenticação da PayFlyPage com uso correto do token via `useAuth()`, remoção do rate-limiting global no Kong (causava 429 em tráfego interno), correção de bugs nos agentes `infrastructure` e `change_mgmt`, adição de índice em `agent_messages(to_agent, status, created_at)` para melhorar performance, ajustes de desempenho.  
>  
> Novas melhorias no módulo **Gastos TI (expenses-service)**: separação entre despesas contratuais e eventuais no PayFly, inclusão de parcelas pendentes, filtro restrito a pagamentos liquidados (`DATALIQUIDACAO IS NOT NULL`), remoção de campo obsoleto `total_pago`, suporte a fornecedores em ambiente de desenvolvimento, correções em filtros de fornecedores (incluindo exclusão de Amazon Plaza, Service e Varejo), otimização no carregamento de dados e melhorias no gráfico de despesas com filtros adicionais e ano padrão definido.  
>  
> Correções adicionais: desativação de alertas WhatsApp no módulo de monitoramento, correções de tipagem TypeScript na `ChangesPage`, e ajustes na triagem de propostas dos agentes com 6 correções aplicadas.