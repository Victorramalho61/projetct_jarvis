...
`app_logger.log_event()` aceita `trace_id: str | None` — usar `current_trace_id.get()` de `main.py` para correlacionar logs entre serviços.

Todos os `main.py` incluem middleware HTTP que gera/propaga `X-Trace-ID` e atualiza CORS para expor esse header.
...
|---|---|---|
| `monitoring` | 15 min | uptime, quality, docker_intel, backend_agent, infrastructure, api_agent, log_scanner, agent_health_supervisor |
| `security` | 30 min | security, code_security |
| `cicd` | 5 min | cicd_monitor |
| `dba` | 4 h | db_dba_agent |
| `governance` | diário (6h) | opportunity_scout, log_strategic_advisor, change_mgmt, itil_version, quality_validator, quality_code_backend, quality_code_frontend, integration_validator, change_validator, scheduling, automation, log_intelligence, log_improver, fix_validator, **proposal_analysis_agent**, proposal_prioritizer, **proposal_supervisor**, llm_manager_agent, cto_assessor_agent |
| `evolution` | diário (7h) | evolution_agent, frontend_agent |
| `documentation` | diário (7h BRT) | docs |

**Timeout global por pipeline**: 15 minutos. Cada agente tem retry automático de 2 tentativas (backoff 3s) antes de marcar erro.

**proposal_analysis_agent** — classifica proposals pendentes por criticidade e tema via keyword matching (determinístico, sem LLM). Roda antes do `proposal_prioritizer` no pipeline governance.

**docs** — movido para pipeline independente `documentation` para não bloquear governance em falhas de GitHub.
